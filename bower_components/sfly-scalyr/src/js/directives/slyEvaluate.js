...
/**
 * Directive for allowing all bound expressions in the current element and its children
 * to be evaluated if the given expression evaluates to true
 * A 'sly-delay' attribute can be set to delay the initial evaluation
 */
.directive('slyEvaluateMe', ['$parse', function ($parse) {
  return {
    restrict: 'A',
    scope: true,
    compile: function compile(tElement, tAttrs) {
      return {
        ...
        pre: function preLink(scope, element, attrs) {
          var firstRun = true;
          var shouldAutoGate = true;
          var expressionToCheck = $parse(attrs['slyEvaluateMe']);
          var delayExpression = (typeof(attrs['slyDelay']) === 'string') ? $parse(attrs['slyDelay']) : null;
          var delay = (delayExpression !== null) ? delayExpression() : 0
          var timeoutId;
          if (delay === 0) {
            shouldAutoGate = false;
          } else {
            element.css('visibility', 'hidden');
            element.on('$destroy', function() {
              clearTimeout(timeoutId);
            });
            timeoutId = setTimeout(function setShouldGate() {
              shouldAutoGate = false;
          ...
          scope.$addWatcherGate(function evalChecker() {
            if(shouldAutoGate === true) {
              return false;
            }
            if(firstRun === true) {
              firstRun = false;
