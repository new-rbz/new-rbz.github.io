var dashboardBase = function (app, headerHeight) {
  ...
  function generateContent() {
    ...
    for (i = 0; i < items.length; i++) {
      ...
      if (items[i].PopUp === 0) {
        $content.append("<div class='tab-pane fade in' id='" + divId
        + "-tab'><div id='buttonBar' class='btn-group btn-group-justified' style='width:100%'>"
        + "<button type='button' style='width:33% !important;' class='btn btn-info btn-sm btn-block' onclick='" + app + ".loadPrevious()'>&lt;- Back</button>"
        + "<button type='button' style='width:33% !important;' class='btn btn-success btn-sm btn-block' onclick='" + app + ".loadNext()'>Forward -&gt;</button>"

        + "<button style='width:34% !important;' type='button' class='btn btn-danger btn-sm btn-block' onclick='"
        + app + ".refreshPage(\"" + divId + "\")'>[Refresh]</button>"

        + "</div><iframe src='" + url + "' id='" + divId
        + "' marginheight='0' frameborder='0' onLoad=\"" + app + ".resizeIframe('" + divId + "');\"></iframe></div>");
      } else {
        windowsToOpen.push(i);
      }
    }

    windowsToOpen.sort(function (a, b) { return a - b });
  ...
  function changeActiveDiv(activeId, oldId) {
    ...
    $("a[href]").parent().removeClass(activeClass);
    $("a[href]").parent().parent().parent().removeClass(activeClass);

    $("a[href='" + newElementId + "']").parent().addClass(activeClass);
    $("a[href='" + newElementId + "']").parent().parent().parent().addClass(activeClass);
  ...
  function generateSimpleMenu($menuElementAnchor) {
    var html = "<ul id='simple' class='nav nav-pills'>", i;

    for (i = 0; i < items.length; i++) {
      html = html + "<li><a data-toggle='tab' href='#" + items[i].div
      + "-tab'>" + items[i].title + "</a></li>";
    }
  ...
  function generateDropdownMenu($menuElementAnchor) {
    ...
    $($menuElementAnchor).prepend("<ul id='top' class='nav nav-tabs'><li class='dropdown'>"
        + "<a class='dropdown-toggle' data-toggle='dropdown' href='#'>"
        + 'Menu' + "<span class='caret'></span></a><ul class='dropdown-menu' id='menu'></ul>");
    ...
    for (count = 0; count < items.length; count++) {
      ...
      $menu.append(root + " href='#" + items[count].div + "-tab'>"
       + items[count].title + "</a></li>");
    }
  }
  ...
  function writeMenuForEachGroup(groups, $menuElementAnchor) {
    ...
    for (var property in groups) {
      if (groups.hasOwnProperty(property)) {
        var group = groups[property], root =
        "<li class='dropdown'><a class='dropdown-toggle' data-toggle='dropdown' href='#'>" + property + "<span class='caret'></span></a><ul class='dropdown-menu'>";

        for (var j = 0; j < group.length; j++) {
          root = root + "<li><a data-toggle='tab' href='#" + group[j].div + "-tab'>" + group[j].title + "</a></li>";
        }
